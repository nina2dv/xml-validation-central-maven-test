...
/**
 * Enhanced implementation for unmarshalling iStar-T XML files to the object model
 * Including support for all NonDecompositionElement types (Quality, Condition/PreBox, IndirectEffect)
 */
public class EnhancedIStarUnmarshaller {
    ...
    public static class IStarUnmarshaller {
        ...
        private Formula convertPreBoxToFormula(PreBoxXml preBoxXml) {
            ...
            if (preBoxXml.getGt() != null) {
                ...
                String left = gtXml.getLeft().getConst();
                String right = gtXml.getRight().getConst();
                return new GTOperator(
                        Formula.createConstantFormula(left),
                        Formula.createConstantFormula(right)
                );
            }
        ...
        private Formula convertFormulaXmlToFormula(FormulaXml formulaXml) {
            ...
            if (formulaXml.getAdd() != null) {
                ...
                if (addXml.getSubtract() != null) {
                    ...
                    Formula left = Formula.createConstantFormula(subtractXml.getLeft().getConst());
                    Formula right = Formula.createConstantFormula(subtractXml.getRight().getConst());
